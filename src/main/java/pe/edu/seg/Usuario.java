...
public class Usuario {
    ...
    public void crear(String id, String psw, String nombre,String tipo) {
        this.id = id;
        this.password = psw;
        this.nombre = nombre;
        this.tipo=tipo;
    ...
    public int getLogueado() throws ClassNotFoundException {
        try {
            ...
            while (resultado.next()) {
                this.tipo = resultado.getString(4).trim();
                contador++;
            }
            if (contador != 0) {
                return 1;
    ...
    public int contarPorTipo(String tipoBuscado) throws ClassNotFoundException {
    int cantidad = 0;
    try {
        Conexion c = new Conexion();
        Connection cnx = c.conecta();
        String query = "SELECT COUNT(*) FROM usuario WHERE tipo = ?";
        PreparedStatement pst = cnx.prepareStatement(query);
        pst.setString(1, tipoBuscado);
    ...
    public LinkedList<Usuario> muestraUsuarios() throws ClassNotFoundException {
        try {
            ...
            while (resultado.next()) {
                ...
                u.tipo=resultado.getString("tipo");
                lista.add(u);
            }
            return lista;
        } catch (SQLException e) {
    ...
    public void agregar(String usr, String psw, String nom, String tpo) throws ClassNotFoundException {
        try {
            Conexion c = new Conexion();
            Connection cnx = c.conecta();
    ...
    public void actualizar(String usr, String psw, String nom, String tpo) throws ClassNotFoundException {
        try {
            Conexion c = new Conexion();
            Connection cnx = c.conecta();
            String query = "update usuario set password=?, nombre=? ,tipo=? where id=?";
            PreparedStatement sentencia = cnx.prepareStatement(query);            
            sentencia.setString(1, psw);
            sentencia.setString(2, nom);
            sentencia.setString(3, tpo);
            sentencia.setString(4,usr);
            sentencia.executeUpdate();
            sentencia.close();
            cnx.close();
        } catch (SQLException e) {
